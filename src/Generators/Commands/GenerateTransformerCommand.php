...
use Aruberuto\Workflow\Generators\TransformerGenerator;
...
class GenerateTransformerCommand extends Command
{
    ...
    {
        try {
            // // Generate create request for transformer
            // $this->call('generate:request', [
            //     'name' => $this->argument('name') . 'Create',
            //     'path' => $this->option('path') . '/Requests'
            // ]);

            // // Generate update request for transformer
            // $this->call('make:aru-request', [
            //     'name' => $this->argument('name') . 'Update',
            //     'path' => $this->option('path') . '/Requests'
            // ]);

            (new TransformerGenerator(array_merge($this->arguments(), $this->options() )))->run();
